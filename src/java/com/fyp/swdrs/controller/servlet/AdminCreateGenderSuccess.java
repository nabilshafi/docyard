...
public class AdminCreateGenderSuccess extends HttpServlet 
{
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
    {
        ...
        try 
        { 
            out.print("<h1>Welcome to adminCreateGender Servlet from doGet</h1>"); 
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
    {
        ...
        try 
        { 
//            out.print("<h1>Welcome to adminCreateGenderSuccess Servlet from doPost</h1>"); 
//            processRequest(request, response);
            String filename = request.getServletContext().getRealPath("/");
            Controller controller = new Controller();
            Object[] returnObj = null;
            Object[] params = new Object[1];
            params[0] = request.getParameter("genderTxt");
//            out.println((String)params[0]);

            returnObj = controller.processRequest(Constants.createGenderAction, params, filename);
            if(returnObj != null)
            {
                if(((String)returnObj[0]).equals("gender"))
                {// Search Result Success
                    ArrayList<String> list = Utility.viewGender(filename);
                    request.getSession().setAttribute("genderList", list);
                    out.println("<body onload=\"myFunction()\">");
                    out.println("</body>");
                    out.println("<script>function myFunction() {alert(\"Gender created Successfully!\");}</script>");
