...
public class AdminCreateSymptomSuccess extends HttpServlet 
{
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
    {
        ...
        try 
        { 
            out.print("<h1>Welcome to adminCreateSymptomSuccess Servlet from doGet</h1>"); 
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
    {
        ...
        try 
        { 
//            out.print("<h1>Welcome to adminCreateSymptomSuccess Servlet from doPost</h1>"); 
//            processRequest(request, response);
            String filename = request.getServletContext().getRealPath("/");
            Controller controller = new Controller();
            Object[] returnObj = null;
            Object[] params = new Object[1];
            params[0] = request.getParameter("symptomTxt");
//            out.println((String)params[0]);

            returnObj = controller.processRequest(Constants.createSymptomAction, params, filename);
            if(returnObj != null)
            {
                if(((String)returnObj[0]).equals("symptom"))
                {// Search Result Success
                    ArrayList<String> list = Utility.viewSymptom(filename);
                    request.getSession().setAttribute("symptomList", list);
                    out.println("<body onload=\"myFunction()\">");
                    out.println("</body>");
                    out.println("<script>function myFunction() {alert(\"Symptom created Successfully!\");}</script>");
