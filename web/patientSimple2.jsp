...
<html lang="en-gb" class="no-js">
    ...
    <body>
        <%
            ArrayList<DoctorProfile> doctorProfiles = (ArrayList<DoctorProfile>)request.getSession().getAttribute("searchSymptomObj");
            ...
            String temp = "";
            if(searchSymptomQuery1 != null)
            {
                temp += searchSymptomQuery1;
                display = "one";
                request.getSession().setAttribute("display", display);
            }
            if(searchSymptomQuery2 != null)
            {
                temp += ", " + searchSymptomQuery2;
                display = "two";
                request.getSession().setAttribute("display", display);
            }
            if(searchSymptomQuery3 != null)
            {
                temp += ", " + searchSymptomQuery3;
                display = "three";
                request.getSession().setAttribute("display", display);
            }
                ...
                <nav class="navbar navbar-inverse" role="navigation">
                    <div id="main-nav" class="collapse navbar-collapse">
                        <ul class="nav navbar-nav" id="mainNav">
                            ...
                            <li>
                                <a href="searchSymptom" class="scroll-link" title="Search">
                                    Search
                                </a>
                            </li>
                            <li>
...
//                                systemRating = decimalFormat.format(systemRating));
                            ...
                            <h4>
                                ...
                                <li><%= hospitalName%> on <%= days%>, at <%= visitingHours%>, located in <%= area%>, <%= city%>, <%= country%>, fee: <%= feeStructure%>.</li> 
                                <%
                                    ...
                                    %>
                                    <li><%= tempHospitalName%> on <%= tempDays%>, at <%= tempVisitingHours%>, located in <%= tempArea%>, <%= tempCity%>, <%= tempCountry%>, fee: <%= tempFeeStructure%>.</li> 
                                <%}%>
                            </h4>
                            <%}%>
                        <%}%>
                    </div>
                    <input id="cancel" class="btn btn-lg btn-primary" type="button" value="Back" onClick="location.href='searchSymptom'" />
                </div>
        </section> 
